class AddTyreMeasurements < ActiveRecord::Migration
  def up
      execute <<-SQL
        INSERT INTO savetyre_tyre_measurements (name,customer_id,created_at,updated_at) VALUES ('UNDEFINED',3,now(),now());
        ...
        INSERT INTO savetyre_tyre_measurements (name,customer_id,created_at,updated_at) VALUES ('UNDEFINED',4,now(),now());
  ...
  def down
    execute <<-SQL
      DELETE FROM savetyre_tyre_measurements WHERE name = 'UNDEFINED' and customer_id = 3;
      ...
      DELETE FROM savetyre_tyre_measurements WHERE name = 'UNDEFINED' and customer_id = 4;
